<div class="nav-bar">
    <div class="start-menus">
        <div class="logo">
            <a href="/store/{{ $store->store_name }}/show">
    ...
    <div class="end-menus">
        <div class="notification">
            <a href="/store/{{ $store->store_name }}/show/messages" target="" rel="noopener noreferrer"> <img
        ...
        <div class="notification">
            <a href="/store/{{ $store->store_name }}/show/notifications" target="" rel="noopener noreferrer"> <img
                    src="data:image/png;base64,{{ $icon['notification'] }}" class="icon-filter" alt="Notification"
                    width="25">
                @if ($storeUser->notifications->where('is_read', 0)->count() > 0)
                    <span class="new-notification">{{ $storeUser->notifications->where('is_read', 0)->count() }}</span>
                @endif
                </span>
            </a>
        </div>
        <div class="notification">
            <a href="/store/{{ $store->store_name }}/show/settings" target="" rel="noopener noreferrer"> <img

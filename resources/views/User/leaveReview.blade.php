@if ($order->status == 'completed')
    ...
    @php
        $review = $order->product->reviews->where('user_id', $user->id)->first();
    @endphp
    <form action="" method="post" style="text-align: center">
        ...
        <div style="margin-bottom:1em;"><strong>Review Type: </strong>
            ...
            <input type="radio" name="review_type" id="positive" value="positive"
                {{ !$review || $review->feedback == 'positive' ? 'checked' : '' }}>
            <label for="neutral">Neutral</label>
            <input type="radio" name="review_type" id="neutral" value="neutral"
                {{ $review && $review->feedback == 'neutral' ? 'checked' : '' }}>
            <label for="negative">Negative</label>
            <input type="radio" name="review_type" id="negative" value="negative"
                {{ $review && $review->feedback == 'negative' ? 'checked' : '' }}>
        </div>

        <textarea name="comment" class="support-msg" id="dispute" cols="30" rows="10"
            placeholder="Leave your review comment here... (max characters: 500)" style="width: 100%;" required>
...
@endif
        ...
        <div style="display:flex; gap:1em;">
            <select name="communication_rating" id="communication_rating" required>
            ...
            <select name="product_rating" id="product_rating" required>
            ...
            <select name="shipping_speed_rating" id="shipping_speed_rating" required>
                <option value="">---Shipping Speed Rating---</option>
            ...
            <select name="price_rating" id="price_rating" required>

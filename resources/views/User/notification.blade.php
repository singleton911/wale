    ...
    <div class="notific-container">
        ...
        @forelse ($user->notifications->sortByDesc('created_at') as $notification)
            <div class="notification-container {{ $notification->is_read ? 'read' : '' }}">
                ...
                <img src="data:image/jpeg;base64,{{ $icon['ads'] }}" alt=""
                    width="30">
                <div class="notification-content">
                    <div style="display: flex;">
                        ...
                        <form action="/notification/{{ $notification->created_at->timestamp }}/{{ $notification->id }}" method="post">
                            @csrf
                            @if (!$notification->is_read)
                                <input type="submit" name="read" id="" class="active" value="Mark as read" style="cursor: pointer;">
                            @endif
                            <input type="submit" name="delete" class="delete" id="" value="Delete" style="cursor: pointer;">
                        </form>
                    </div>
                    <p class="notification-message">{{ $notification->notificationType->content }}
                    @if ($notification->notificationType->icon == 'order')
                        <a href="/order/{{ $notification->order->created_at->timestamp }}/{{ $notification->option_id }}" style="font-size:1rem; text-decoration:underline;">see order details here</a>
                    @endif
                    </p>
                </div>
            </div>
